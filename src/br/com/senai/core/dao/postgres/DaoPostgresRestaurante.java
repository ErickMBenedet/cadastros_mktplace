...
public class DaoPostgresRestaurante implements DaoRestaurante {
	...
	private final String INSERT = "INSERT INTO restaurantes (nome, descricao, cidade, logradouro, bairro, complemento, id_categoria)" 
													+"VALUES (?, ?, ?, ?, ?, ?, ?)";
	private final String UPDATE = "UPDATE restaurantes" 
													+"SET nome = ?, descricao = ?, cidade = ?, logradouro = ?, bairro = ?, complemento = ?, id_categoria = ?"
													+"WHERE id = ? ";	
	private final String DELETE = "DELETE FROM restaurantes" 
													+ "WHERE id = ?";
	private final String SELECT_BY_ID = "SELECT r.id id_restaurante, r.nome nome_restaurante, r.descricao, r.cidade, r.logradouro, r.bairro, r.complemento, c.id id_categoria, c.nome nome_categoria"
																+ "FROM restaurante r, categoria c"
																+ "WHERE r.id_categoria = c.id "
																+ "AND r.id";
	private final String SELECT_BY_NOME_CATEG = "SELECT r.id id_restaurante, r.nome nome_restaurante, r.descricao, r.cidade, r.logradouro, r.bairro, r.complemento, c.id id_categoria, c.nome nome_categoria"
																					+ "FROM restaurante r, categoria c"
																					+ "WHERE r.id_categoria = c.id ";
	...
	public void inserir(Restaurante restaurante) {
		...
		try {
			...
			throw new RuntimeException("Ops! Houve um erro ao inserir o restaurante. Motivo: " + e.getMessage());
		}finally {
	...
	public void alterar(Restaurante restaurante) {
		PreparedStatement ps = null;
			...
			try {
				...
				throw new RuntimeException("Eita! Houve um erro ao alterar o restaurante. Motivo: " + e.getMessage());
			}finally {
				ManagerDb.getInstance().fechar(ps);
			}
	}
	...
	public void excluirPor(int id) {
		...
		try {
			...
			throw new RuntimeException("Putz! Houve um erro ao excluir o restaurante. Motivo: " + e.getMessage());
		}finally {
			ManagerDb.getInstance().fechar(ps);
		}
	...
	public Restaurante buscarPor(int id) {
		...
		try {
			...
			throw new RuntimeException("Houve um erro ao buscar o restaurante. Motivo: " + e.getMessage());
		}finally {
			ManagerDb.getInstance().fechar(ps);
	...
	public List<Restaurante> listarPor(String nome, Categoria categoria) {
		...
		try {
			...
			throw new RuntimeException("Houve um erro ao listar os restaurantes. Motivo: " + e.getMessage());
		}finally {
			ManagerDb.getInstance().fechar(ps);
			ManagerDb.getInstance().fechar(rs);
		}
	...
	private Restaurante extrairDo(ResultSet rs) {
		try {
			...
			return new Restaurante(nomeDoRestaurante, descricao, endereco, categoria);
			
		}catch (Exception e) {
			throw new RuntimeException("Epa! Ocorreu um erro ao extrair o restaurante. Motivo: " + e.getMessage());
		}
	}
