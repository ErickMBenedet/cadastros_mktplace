...
public class DaoPostgresCategoria implements DaoCategoria {
	...
	private final String INSERT = "INSERT INTO categorias (nome) "
			                                      	+ "VALUES (?)";
	private final String UPDATE = "UPDATE categorias SET nome = ? "
													 + "WHERE id = ?";
	private final String DELETE = "DELETE FROM categorias "
													 + "WHERE id = ?";
	...
	public void inserir(Categoria categoria) {
		...
		try {
			...
			throw new RuntimeException("Ops! Houve um erro ao inserir a categria. Motivo: " + e.getMessage());
		}finally {
	...
	public void alterar(Categoria categoria) {
		...
		try {
			...
			throw new RuntimeException("Eita! Houve um erro ao alterar a categria. Motivo: " + e.getMessage());
		}finally {
		ManagerDb.getInstance().fechar(ps);
		}
	...
	public void excluirPor(int id) {
		...
		try {
			...
			throw new RuntimeException("Putz! Houve um erro ao excluir a categria. Motivo: " + e.getMessage());
		}finally {
			ManagerDb.getInstance().fechar(ps);
		}
	...
	public List<Categoria> listarPor(String nome) {
		...
		try {
			...
			throw new RuntimeException("Ocorreu um erro ao listar categorias. Motivo: " + e.getMessage());
		}finally {
			ManagerDb.getInstance().fechar(ps);
			ManagerDb.getInstance().fechar(rs);
		}
	...
	private Categoria extrairDo(ResultSet rs) {
		try {
			...
			throw new RuntimeException("Epa! Ocorreu um erro ao listar a categoria. Motivo: " + e.getMessage());
		}
	}
